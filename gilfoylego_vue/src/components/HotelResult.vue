<template>
  <div>
    <div
      class="container my-4 p-5 grey-border custom-link"
      v-for="hotel in filteredHotels"
      :key="hotel.id"
      v-on:click="resultLink(hotel)"
    >
      <div class="row result w-100">
        ...
        <div class="col-sm-12 col-md-12 col-lg-8 px-4">
          ...
          <div class="row">
            <span>
              <em>{{hotel.city}}, {{hotel.country}}</em>
            </span>
          </div>
          <div class="row">
          ...
          <div class="row d-flex justify-content-end pt-5">Från hotel.rooms[0].price kr</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    formAmenities() {
      return [
        this.$store.state.form.amenities.includes("eveningentertainment"),
    ...
    filteredHotels: function() {
      ...
      if (this.$store.state.form.m2Beach > 0) {
        searchResult = searchResult.filter(
          hotel => hotel.mToBeach <= this.$store.state.form.m2Beach
        );
      }

      if (this.$store.state.form.m2Center > 0) {
        searchResult = searchResult.filter(
          hotel => hotel.mToCity <= this.$store.state.form.m2Center
        );
      }

      searchResult.forEach(hotel => {
        if (!hotel.rooms || !hotel.rooms.push) {
          return;
        }
      ...
      if (this.$store.state.sortByRatings) {
        ...
        if (!searchResult || !searchResult.push) {
          return [];
        }
        return searchResult.sort(
          (a, b) => ascDesc * a.rating.toString().localeCompare(b.rating)
        );
      }
      ...
      if (this.$store.state.sortByPrice) {
        ...
        if (searchResult && searchResult.push) {
          searchResult.sort((a, b) => {
            ...
            if (!a.rooms || !a.rooms[0]) {
              a.rooms = [{ price: 0 }];
            }
            if (!b.rooms || !b.rooms[0]) {
              b.rooms = [{ price: 0 }];
            }
